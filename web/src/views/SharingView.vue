...
async function submitContribute() {
  ...
  try {
    ...
    // Error already surfaced via uploadError
...
async function loadArchives() {
  ...
  try {
    ...
    // Silently handle load failure — UI shows empty state
  } finally {
    loading.value = false;
  }
}
...
// ─── Admin: Preview / Edit (delegated to child components) ────────────────────
